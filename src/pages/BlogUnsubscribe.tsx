...
export default function BlogUnsubscribe() {
  ...
  onMount(async () => {
    ...
    if (!token) {
      setLoading(false);
      setMessage({
        text: "Avmeldingslenkja er ugyldig.",
        type: "error",
      });
      return;
    }
    ...
    try {
      const res = await fetch(
        `${mailerUrl}/unsubscribe?token=${encodeURIComponent(token)}`,
      );
      if (res.status === 200) {
        setMessage({
          text: "Abonnementet ditt er avslutta. Du vil ikkje lenger mottaka e-post frå oss.",
          type: "success",
        });
        ...
        setMessage({
          text: "Kunne ikkje finne abonnementet ditt. Har du allereie avslutta?",
          type: "error",
        });
      } else {
        setMessage({
